      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {isLoading ? (
          ...
          data.map((item) => (
            <article
              key={item._id}
              className="relative rounded-2xl border border-slate-200/70 bg-white p-5 dark:border-white/10 dark:bg-slate-900"
            >
              <p className="font-semibold">{item.name}</p>
              <p className="mt-4 text-2xl font-bold">{config.display(item)}</p>
              <button
      ...
      api.post(
        ...
        Object.fromEntries(
          Object.entries(values).map(([key, value]) =>
            ['balance', 'principal', 'outstanding', 'emiAmount'].includes(key)
              ? [key, Number(value)]
              : [key, value]
          )
        )
